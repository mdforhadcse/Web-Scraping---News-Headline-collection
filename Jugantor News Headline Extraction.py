...
def generate_filename(search_term):
    ...
    filename = 'Jugantor_'+stem + '_' + timestamp + '.csv'
    return filename


def save_data_to_csv(record, filename, new_file=False):
    header = ['headline', 'link', 'time']
    if new_file:
...
def generate_url(search_term, page):
    base_template = 'https://www.jugantor.com/all-news/country-news/{}'
...
def extract_card_data(card):
    headline = card.find('h6').text.strip()
    link = card.find('a')['href'].strip()
    time = card.getText('span').split("\n")[4].replace('span', '')[2:].strip()
    return headline, link, time


def collect_product_cards_from_page(driver):
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    cards = soup.find_all('div', attrs={'class': 'col-8 align-self-end pl-4'})
    return cards
...
def run(search_term):
    ...
    for page in range(1, 9):  # max of 20 pages
        # load the next page
    ...
    driver.quit()
    print(f"Scraped {num_records_scraped:,d} for the search term: {search_term} from Jugantor")


if __name__ == '__main__':
    term = 'barisal patuakhali dumki'
    run(term)
